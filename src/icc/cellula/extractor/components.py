...
class LibExtractorExtractor(object):
    ...
    def __init__(self):
        ...
        if not self.test_working():
            raise RuntimeError(
                'cannot determine operability of extract utility')

        tmp_path = config[self.config_section].get('tmp_path',
                                                   tempfile.gettempdir())
    ...
    def do_extract(self, filepath):
        ...
        for line in lines:
            ...
            if len(comps) == 0:
                continue
            if len(comps) == 1:
                logger.warning("Tracker: found a 1-part semantic component: " +
                               comps[0][:60])
                continue
            p, o = comps
            if p == 'a':
...
class RecollExtractor(object):
    ...
    def __init__(self):
        ...
        self.tmp_path = config[self.config_section].get('tmp_path',
                                                        tempfile.gettempdir())
        ...
        if way.startswith(
            ...
            try:
                ...
                out = self.run(tmpfile,
                               executable=cmd,
                               encoding=meta.get("text|charset", 'utf-8'))
        else:
            logger.warning('Non-implemented filer class %s for script %s' %
                           (way, script))
        out = out.strip()
        if out:
            meta['text-body'] = out
